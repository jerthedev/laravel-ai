...
class WritePerformanceTest extends TestCase
{
    ...
    #[Test]
    #[Group('performance')]
    public function it_measures_ai_usage_costs_insert_performance_improvement(): void
    {
        // Measure baseline performance with original indexes (simulated)
        $baselineTime = $this->measureBaselineInsertPerformance('ai_usage_costs');
        
        // Measure current performance with consolidated indexes
        $currentTime = $this->measureCurrentInsertPerformance('ai_usage_costs');
    ...
    public function it_validates_bulk_insert_performance(): void
    {
        ...
        foreach (array_chunk($usageCosts, self::BATCH_SIZE) as $chunk) {
            DB::table('ai_usage_costs')->insert($chunk);
        }
        ...
        $this->assertEquals(self::BATCH_SIZE * 5, DB::table('ai_usage_costs')->count());
    }
    ...
    public function it_validates_concurrent_write_performance(): void
    {
        ...
        for ($i = 0; $i < $concurrentBatches; $i++) {
            ...
            // Insert each batch
            DB::table('ai_usage_costs')->insert($batchData);
        }
        ...
        // Verify all records were inserted
        $this->assertEquals($totalRecords, DB::table('ai_usage_costs')->count());
    }
    ...
    public function it_measures_update_performance_improvement(): void
    {
        ...
        DB::table('ai_usage_costs')->insert($initialData);
        
        $startTime = microtime(true);
        
        // Perform batch updates
        DB::table('ai_usage_costs')
        ...
        $updatedCount = DB::table('ai_usage_costs')
            ->where('provider', 'openai')
            ->count();
    ...
    public function it_measures_delete_performance_with_optimized_indexes(): void
    {
        ...
        DB::table('ai_usage_costs')->insert($testData);
        
        $startTime = microtime(true);
        
        // Delete using indexed column (should be fast)
        $deletedCount = DB::table('ai_usage_costs')
    ...
    private function addTemporaryIndexes(string $table): void
    {
        switch ($table) {
            case 'ai_usage_costs':
                DB::statement('CREATE INDEX temp_user_id ON ai_usage_costs (user_id)');
                DB::statement('CREATE INDEX temp_provider ON ai_usage_costs (provider)');
                DB::statement('CREATE INDEX temp_model ON ai_usage_costs (model)');
                DB::statement('CREATE INDEX temp_conversation_id ON ai_usage_costs (conversation_id)');
                break;
