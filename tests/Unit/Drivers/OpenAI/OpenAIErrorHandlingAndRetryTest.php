 ...
 * rate limit handling, and exception mapping for OpenAI driver.
 */
#[Group('unit')]
#[Group('openai')]
#[Group('error-handling')]
#[Group('retry')]
class OpenAIErrorHandlingAndRetryTest extends TestCase
{
    ...
    public function it_maps_invalid_api_key_error_to_credentials_exception(): void
    {
        ...
        $this->driver->sendMessage($message, ['model' => 'gpt-3.5-turbo']);
    }
    ...
    public function it_maps_quota_exceeded_error_to_quota_exception(): void
    {
        ...
        $this->expectException(OpenAIQuotaExceededException::class);
    ...
    public function it_maps_server_error_to_server_exception(): void
    {
        ...
        $noRetryDriver = new OpenAIDriver([
            'api_key' => 'sk-test-key-for-unit-tests',
        ...
        $this->expectException(OpenAIServerException::class);
        $this->expectExceptionMessage('server error');

        $message = AIMessage::user('Test message');
        $noRetryDriver->sendMessage($message, ['model' => 'gpt-3.5-turbo']);
    }
    ...
    public function it_retries_on_retryable_errors(): void
    {
        ...
        $message = AIMessage::user('Test message');
        $response = $this->driver->sendMessage($message, ['model' => 'gpt-3.5-turbo']);
    ...
    public function it_uses_custom_retry_configuration(): void
    {
        ...
        $message = AIMessage::user('Test message');
        $customDriver->sendMessage($message, ['model' => 'gpt-3.5-turbo']);
    }
    ...
    public function it_handles_streaming_errors_with_retry(): void
    {
        ...
        foreach ($this->driver->sendStreamingMessage($message, ['model' => 'gpt-3.5-turbo']) as $chunk) {
            $chunks[] = $chunk;
        }
    ...
    /**
     * Create a mock OpenAI API response for successful requests.
     */
    ...
    {
        return [
            'invalid_api_key' => ['Invalid API key provided', OpenAIInvalidCredentialsException::class],
            'authentication_error' => ['Authentication failed', OpenAIInvalidCredentialsException::class],
            'rate_limit_exceeded' => ['Rate limit exceeded', OpenAIRateLimitException::class],
            'quota_exceeded' => ['You exceeded your current quota', OpenAIQuotaExceededException::class],
            'insufficient_quota' => ['Insufficient quota', OpenAIQuotaExceededException::class],
            'server_error' => ['Internal server error', OpenAIServerException::class],
            'service_unavailable' => ['Service temporarily unavailable', OpenAIServerException::class],
            'timeout' => ['Request timeout', OpenAIException::class],
            'unknown_error' => ['Unknown error occurred', OpenAIException::class],
        ];
    }
    ...
    public function it_enhances_error_messages_with_helpful_context(): void
    {
        ...
        try {
            ...
            $this->assertStringContainsString('Invalid OpenAI API key', $message);
